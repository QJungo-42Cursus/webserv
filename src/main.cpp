...
static void readSocket(int fd, Config *configFromFd[], t_fdSets *fdSets, Client *clientArray[])
{
    ...
    int bufSize = MAX_HEADER_SIZE + MAX_BODY_SIZE +
                  1; // shld replace max body size by config value (should also protect against stack overflow?)
...
static void processRequest(Client *client, Config *config)
{
    ...
    std::cout << "==== Prepared response ====" << std::endl << response_string << "\n=================="
              << std::endl; //tmp for debug
    client->setResponse(response_string);
